...
renderCart = () => {
  ...
  holder.innerHTML = cart.length ? cart.map(item => {
    ...
    return `<div class="cart-row"><img src="${product.image}" alt=""><div><strong>${product.name}</strong>${variant}<br><small>Quantità: ${item.quantity} · ${formatPrice(product)}</small></div><button aria-label="Rimuovi ${product.name}" data-remove-key="${cartItemKey(item)}">×</button></div>`;
